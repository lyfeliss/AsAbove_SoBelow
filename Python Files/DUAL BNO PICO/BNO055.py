...
class BNO055:
    ...
    def read_quat(self):
        ...
        z *= scale
        return (w, x, y, z)
    REG_MAG_DATA = 0x0E  # 6 bytes for X, Y, Z

    def read_mag(self):
        """
        Reads raw magnetometer data (X, Y, Z).
        Check the datasheet or the BNO055 manual
        for the exact scaling factor in the selected units.
        """
        data = self.i2c.readfrom_mem(self.address, self.REG_MAG_DATA, 6)
        mx = self._to_signed_16(data[0] | (data[1] << 8))
        my = self._to_signed_16(data[2] | (data[3] << 8))
        mz = self._to_signed_16(data[4] | (data[5] << 8))

        # According to the BNO055 datasheet:
        # If the UNIT_SEL (0x3B) is 0x00 (the default), magnetometer data is in microteslas,
        # with 1 LSB = 0.0625 µT. That means we should multiply raw by 0.0625 to get µT:
        scale = 0.0625
        mx *= scale
        my *= scale
        mz *= scale
